...
void tcp_packet_handler(unsigned char *, const struct pcap_pkthdr *, const unsigned char *);
...
int main(int argc, char *argv[])
{
  ...
  // Inicio de programa.
  std::cout << "Práctica 14" << std::endl;
  std::cout << "Analizador de TCP" << std::endl;
  ...
  if (pcap_compile(ad_handle, &fp, "tcp", 0, pNet) == -1)
  {
  ...
  // Realizar la captura.
  pcap_loop(ad_handle, number_of_packets, &tcp_packet_handler, NULL);

  // Informar sobre fin de programa.
  std::cout << "Fin de programa" << std::endl;
  std::cout << " - Humberto Alcocer, 2019" << std::endl;
  std::cout << "ESCOM. Redes de Computadoras. Práctica 14" << std::endl;
...
void tcp_packet_handler(unsigned char *param, const struct pcap_pkthdr *header, const unsigned char *pkt_data)
{
  ...
  if (type == 2048)
  {
    ...
    if (ip->proto == 6)
    {
      ...
      std::cout << "· Análisis de TCP:" << std::endl;

      // Analizar TCP.
      tcph *tcp = (tcph *)(pkt_data + 14 + hlen);

      // Puerto de origen
      std::cout << "  ~ Puerto de Origen: " << tcp->sport << std::endl;
      // Puerto destino
      std::cout << "  ~ Puerto de Destino: " << tcp->dport << std::endl;
      // Número de secuencia.
      std::cout << "  ~ Número de secuencia: " << tcp->nsec << std::endl;
      // Número de ack.
      std::cout << "  ~ Número de ack: " << tcp->nack << std::endl;
      // Longitud.
      std::cout << "  ~ Longitud: " << tcp->hlen0 << std::endl;
      // Banderas.
      std::cout << "  ~ Banderas: " << std::endl;
      std::cout << "    + CWR: " << ((tcp->flags & 0x01) ? "Encendido" : "Apagado") << std::endl;
      std::cout << "    + ECE: " << (((tcp->flags >> 1) & 0x01) ? "Encendido" : "Apagado") << std::endl;
      std::cout << "    + URG: " << (((tcp->flags >> 2) & 0x01) ? "Encendido" : "Apagado") << std::endl;
      std::cout << "    + ACK: " << (((tcp->flags >> 3) & 0x01) ? "Encendido" : "Apagado") << std::endl;
      std::cout << "    + PSH: " << (((tcp->flags >> 4) & 0x01) ? "Encendido" : "Apagado") << std::endl;
      std::cout << "    + RST: " << (((tcp->flags >> 5) & 0x01) ? "Encendido" : "Apagado") << std::endl;
      std::cout << "    + SYN: " << (((tcp->flags >> 6) & 0x01) ? "Encendido" : "Apagado") << std::endl;
      std::cout << "    + FIN: " << (((tcp->flags >> 7) & 0x01) ? "Encendido" : "Apagado") << std::endl;
      // Ventana.
      std::cout << "  ~ Ventana: " << tcp->window << std::endl;
      // Checksum
    ...
    {
      std::cout << "· El paquete IP no contiene un paquete TCP." << std::endl;
    }
  }
  else
  {
