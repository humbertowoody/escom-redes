...
int main(int argc, char *argv[])
{
  ...
  // Inicio de programa.
  std::cout << "Práctica 13" << std::endl;
  std::cout << "Analizador de IGMP" << std::endl;
  ...
  if (pcap_compile(ad_handle, &fp, "igmp", 0, pNet) == -1)
  {
...
void ip_packet_handler(unsigned char *param, const struct pcap_pkthdr *header, const unsigned char *pkt_data)
{
  ...
  if (type == 2048)
  {
    ...
    if (ip->proto == 2)
    {
      ...
      std::cout << "· Análisis de IGMP:" << std::endl;
      ...
      case 0x11:
        ...
        break;
      case 0x12:
        std::cout << "reporte (IGMPv1)." << std::endl;
        break;
      case 0x16:
        std::cout << "reporte (IGMPv2)." << std::endl;
        break;
      case 0x22:
        std::cout << "reporte (IGMPv3)." << std::endl;
        break;
      default:
        std::cout << "desconocido (" << igmp->type << ")." << std::endl;
      }

      // Tiempo.
      std::cout << "  ~ Tiempo: " << static_cast<unsigned>(igmp->time) << "ms" << std::endl;
    ...
    {
      std::cout << "· El paquete IP no contiene un paquete IGMP." << std::endl;
    }
  }
  else
  {
